...
## AADX semantic object hierarchy
...
A `block` is used to wrap subordinate `resourceS` and thereby provide a means of annotating them. There are many subclasses of resource which can be wrapped, such as: `dataset`, `operator`, `model`, and `application`.
...
!!! tip
    Do not be put off by the complexity of some object defintions. In reality, even when concentiously creating a thoroughly described object, it is unlikely that a user will need to define every property.
 
### Space schema

???+ code "Schema definition for a space"
    ```json
     {
...
???+ code "Schema definition for a block"
    ```json
    {
...
???+ code "Schema definition for a dataset"
    ```json
    {
...
![Creating a space via the webapp](img/create-space-webapp.png)

/// caption
Creating a space using the AADX webapp
///

A new space will contain no datasets and consequently will appear as follows.

![A new empty space](img/new-empty-space.png)

/// caption
A new space, just after creation
///
...
![Uploading a file via the webapp](img/upload-file-webapp.png)
...
Create a new API key pair by clicking on the "+ Create new key" button. You'll be asked to give the key pair a name. Follow the instructions to keep a record of both the Key ID and Secret, for later use in the API. 

!!! tip
    You can create new key pairs at any time.

![Creating an API key pair](img/create-api-key.png)

/// caption
Creating API keys for use with the Workspace REST API
///
...
For instance, using the Python `#!py3 requests` library we just supply an instance of HTTPBasicAuth when making calls to a Workspace API endpoint.

```python3 title="Making authenticated requests"
import requests
...
!!! warning
    In the following examples, it is assumed that we will use the boiler plate above to authenticate calls to the API.
...
```python3 title="Creating a simple dataset"
...
```python3 title="Querying the platform for a relevant creator"
...
```python3 title="Wrapping the dataset in a block"
payload = {
...
A `block` can be added to a `space` using the the Workspace API, however `blockS` are not added directly to the space itself, but rather a `collection` resource to which the `space` is linked. Like all resources, `collectionS` are defined by a schema (`ern:e-pn.io:schema:collection`), but these details are not necessary for our task, only the ERN of the `collection` associated with our `space`.

We can get the ERN of our chosen `space` very easily using the web application.

![Copy space ERN to clipboard](img/copy-space-ern.png)

/// caption
Copy space ERN to clipboard
///
...
```python3 title="Adding our block to the collection"
payload = {
	"action": "AddItems",
	"parameters": {
